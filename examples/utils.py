...
def render_image_test(
    ...
    def sigma_fn(t_starts, t_ends, indices, mask):
        ray_indices = total_alive_indices[mask][indices]
    ...
    def rgb_sigma_fn(t_starts, t_ends, indices, mask):
        ray_indices = total_alive_indices[mask][indices]
        t_origins = rays.origins[ray_indices]
        t_dirs = rays.viewdirs[ray_indices]
        positions = t_origins + t_dirs * (t_starts + t_ends) / 2.0
    ...
    total_alive_indices = torch.arange(N_rays, device=device)

    min_samples = 1 if cone_angle==0 else 8
    ...
    while samples < max_samples:
        ...
        t_min_mask = t_min[mask]

        N_alive = len(alive_indices)
        if N_alive==0: break

        # the number of samples to add on each ray
        N_samples = max(min(N_rays//N_alive, 16), min_samples)
        samples += N_samples
        ...
        rgb, opacity, depth, _ = rendering_test(
            mask,
            opacities[mask],
            t_starts,
            t_ends,
...
def render_image_test_v2(
    ...
    for i in range(0, num_rays, chunk):
        chunk_rays = namedtuple_map(lambda r: r[i : i + chunk], rays)
        ray_indices, t_starts, t_ends = ray_marching_test_v2(
            max_samples,
            chunk_rays.origins,
            chunk_rays.viewdirs,
            scene_aabb=scene_aabb,
        ...
        results.append(chunk_results)
        packed_info = pack_info(ray_indices, n_rays=len(chunk_rays.origins))
        extra_info.append((weight, t_starts, t_ends, ray_indices, packed_info))
    colors, opacities, depths, n_rendering_samples = [
        torch.cat(r, dim=0) if isinstance(r[0], torch.Tensor) else r
        for r in zip(*results)
    ]
